...
namespace st {
    void KeyboardMovementController::moveInPlaneXZ(
        GLFWwindow* window, float dt, StGameObject& gameObject) {
        glm::vec3 rotate{0};
        ...
        if (glm::dot(rotate, rotate) > std::numeric_limits<float>::epsilon()) {
            gameObject.transform3d.rotation += lookSpeed * dt * glm::normalize(rotate);
        }
        ...
        if (glm::dot(moveDir, moveDir) > std::numeric_limits<float>::epsilon()) {
            gameObject.transform3d.translation += moveSpeed * dt * glm::normalize(moveDir);
        }
    }
}  // namespace lve